...
const addResume= useCallback(() => {
  ...
  phone.trim() !== '' &&degree.trim() !== '' &&
  address.trim() !== '' &&postalCode.trim() !== '' && city.trim() !== '' &&countryCode.trim() !== '' ) {
  let newResume = {};
//set Basic  
  let newLocation = {}; 
  ...
  let newProfile = {};
  newProfile.network = network;
  newProfile.url = url;
  
  let newBasic={};
  newBasic.name = name;
  newBasic.picture = picture;
  newBasic.email = email;
  newBasic.phone = phone;
  newBasic.degree = degree;
  newBasic.location = newLocation;
  newBasic.profiles = newProfile;
//set resume
  newResume.basics = newBasic;
  newResume.work = work;
  newResume.volunteer = volunteer;
  newResume.education = education;
  newResume.awards = awards;
  newResume.publications = publications;
  newResume.skills = skills;
  newResume.languages = language;
  newResume.interests = interests;
  setResume(newResume=>[...resume,newResume]);
  ...
  network, url, name, picture, email, phone, degree,
  work, volunteer, education, awards, publications,
  skills, language, interests
]);
...
<div className="min-h-screen bg-gray-200 w-screen"> 
  <div className="bg-gray-700 h-24 border-b-2 border-white p-2 text-white">
    ...
    <div className="font-thin">Please complete the form below</div>
  </div>
  ...
  <div className="grid grid-cols-4 grid-rows-3 gap-2 h-96 p-10 bg-slate-300 border rounded-2xl relative top-5">
    ...
    <div className="row-start-1 row-span-3 col-span-2 col-start-3 ">
      <div>{education.map((item, index) => (<div className="grid grid-rows-3 bg-gradient-to-b from-slate-800 to-gray-700 mb-4 relative top-16 rounded text-white">
        ...
        <div className="relative left-2 font-mono">From {item.educationStartDate} to {item.educationEndDate}</div>
        </div>))}
      </div>
    </div>
  </div>
    ...
    <div className="row-span-4 col-start-2 text-black">
      <div>
        {awards.map((item, index) => (
          ...
          <div className="font-normal">{item.awarder}</div>
          <div>{item.date}</div>
          </div>))}
      </div>
    </div>
    ...
    <div className="row-span-6 col-start-2 ">
      <div>
        {publications.map((item, index) => (
          <div className="grid grid-rows-3 m-2 bg-white rounded" key={index}>
          <div className="text-xl font-bold">{item.publicationName} </div>
          <div className="font-normal">{item.publisher}</div>
  ...
  <div className="grid grid-rows-1 relative left-96 bg-gradient-to-br from-emerald-500 to-green-600 text-white -top-4 w-72  rounded h-10">       
  <button onClick={addResume}
  className="hover:opacity-50 focus:outline-none focus:ring focus:ring-white"
  > Sumbit</button>     
  </div>        
